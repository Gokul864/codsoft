import gokul as gk
from gokul import messagebox as mb

# Custom Button class
class LargeButton(gk.Button):
...
class Calculator:
    def __init__(self, master, title, icon_path):
        ...
        # Create widgets
        self.frame = gk.Frame(master, background='black')
        self.label = gk.Label(self.frame, textvariable=self.equation_label, font='Helvetica 32 bold', fg='white', bg='black', height=1, width="10", anchor='e')

        self.btn_ac = gk.Button(self.frame, text='AC', command=self.all_clear, bg='black', fg='red', font="Helvetica 15", border=0, height=1, width=6, activebackground='black', activeforeground='white')
        self.btn_back = gk.Button(self.frame, command=self.backspace, border=0, bg='black', fg="white", text="<", font='Helvetica 15 bold', width=6, activebackground='black', activeforeground='white')
